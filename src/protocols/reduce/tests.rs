...
impl Handler<ProtocolFinished<ProtocolDataType>> for TestClusterMemberListener {
    ...
    fn handle(
        &mut self,
        msg: ProtocolFinished<ProtocolDataType>,
...
fn reduce_correct_value() {
    let main_addr = format!("127.0.0.1:{}", request_open_port().unwrap_or(1992))
        .parse()
        .unwrap();
    let sub_addr = format!("127.0.0.1:{}", request_open_port().unwrap_or(1993))
        .parse()
        .unwrap();
    ...
    params
        .into_par_iter()
        .for_each(|(v, h, s, m, e)| run_single_reduction_worker(v, h, s, m, e));
    let received = (&*(expected_value_arc.lock().unwrap()));
    assert_eq!(expected_value, received.as_ref().unwrap());
}

#[actix_rt::main]
async fn run_single_reduction_worker(
    ...
    let _cluster_listener =
        TestClusterMemberListener::new(host.eq(&main), main, 2, host, value, expected).start();
