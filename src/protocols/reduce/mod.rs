...
pub trait Reduce {
    fn reduce(&self, value: ProtocolDataType, reduce_operation: ReduceOperation, receiver_node_id: usize, protocol_id: &str);
    fn reduce_to_main(&self, value: ProtocolDataType, reduce_operation: ReduceOperation, protocol_id: &str);
}
...
impl ReduceOperation {
    pub fn apply(&self, values: Vec<ProtocolDataType>) -> ProtocolDataType {
        let shape = values.first().expect("Should at least have one element").shape();
        match self {
            ReduceOperation::Sum => {
